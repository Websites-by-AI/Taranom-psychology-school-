...
export default function ReportCardView({ student, onNavigate }: ReportCardViewProps) {
    ...
    try {
      ...
      const localWeaknesses = weakSubjects.map((sub) => {
        ...
        if (sub.lessonName.includes("زیست‌شناسی")) {
          ...
          questions = 35;
          severity = sub.percentage < 40 ? "critical" : "warning";
        } else if (sub.lessonName.includes("جامعه‌شناسی")) {
          topic = "جهان‌های فرهنگی و هوش هنجاری اجتماعی";
          rec = "مطالعه دقیق تبارشناسی جهان مادی‌گرای مدرن؛ مرور ۲۵ تست خط به خط متن کتاب درسی.";
          questions = 25;
          severity = sub.percentage < 40 ? "critical" : "warning";
        }
      ...
      setRemedialPlan([
        { day: "شنبه", morningPlan: `مطالعه متون و مفاهیم کتاب درسی با تمرکز بر مباحث سنگین`, afternoonPlan: "انجام ۱۵ تست تشریحی و مانیتورینگ تله‌های گزینه‌ای", totalQuestions: 15 },
        { day: "یکشنبه", morningPlan: `تمرین روی محاسبات دسته‌بندی شده سرعت بالا`, afternoonPlan: "تست‌زنی جامع ۲۰ سوال موضوعی کنکور سال‌های قبل", totalQuestions: 20 },
        { day: "دوشنبه", morningPlan: "مرور خلاصه‌نویسی‌ها و مینی‌چارت‌های مربی‌گری", afternoonPlan: "آزمون مبحثی کایزن روی فصول چالش برانگیز", totalQuestions: 30 },
        { day: "سه‌شنبه", morningPlan: `رفع اشکال تستی عمیق روی سوالات غلط آزمون اخیر`, afternoonPlan: "حل و تحلیل تستی ۲۵ تست شبیه‌ساز ترنم مهر", totalQuestions: 25 },
        { day: "چهارشنبه", morningPlan: `پیش‌خوانی مباحث جدید همراه با رسم درخت مفاهیم`, afternoonPlan: "آزمون ۲ ساعته جامع و همه‌جانبه مشابه با شرایط کنکور", totalQuestions: 35 },
        { day: "پنجشنبه", morningPlan: "رفع اشکال تله‌های آزمون طبقه‌بندی شده گذشته", afternoonPlan: "ثبت ساعات مطالعه و بررسی روند بهینه خستگی علمی", totalQuestions: 10 },
        { day: "جمعه", morningPlan: "کارگاه آنلاین و وبینار ارشد با مشاور برتر کنکور ترنم مهر", afternoonPlan: "تحلیل زنده کارنامه آزمون آزمایشی با مربی تخصصی", totalQuestions: 40 }
      ]);
  ...
  useEffect(() => {
    ...
    if (testPhase === "running" && testTimer > 0) {
      timerId = setInterval(() => {
        setTestTimer(prev => {
          if (prev <= 1) {
  ...
  const handleStartCustomTest = () => {
    ...
    const pool: BuiltQuestion[] = [
      ...
      },
      {
        ...
        correctIdx: 0,
        explanation: "موازنه درست واکنش: C3H8 + 5 O2 -> 3 CO2 + 4 H2O است. مول‌های واکنش دهنده ۶ و فرآورده ۷ مول است (تفاضل = ۱ مول). در آزمون‌ها، داوطلبین پروپان یا حالات فیزیکی فرآورده را به غلط لحاظ می‌کنند.",
    ...
    // Filter questions that match selectedCustomSubjects
    let filtered = pool.filter(q => selectedCustomSubjects.includes(q.subject));
    if (filtered.length === 0) {
      filtered = pool.slice(0, Math.min(pool.length, testQuestionsCount));
    }
