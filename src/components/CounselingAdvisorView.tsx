...
export default function CounselingAdvisorView({ student }: CounselingAdvisorViewProps) {
  ...
  return (
    <div className="space-y-8 animate-fade-in text-right" style={{ direction: "rtl" }}>
      {/* Hero Section */}
      <div className="bg-slate-950 rounded-[40px] p-10 text-white relative overflow-hidden shadow-2xl border border-white/5">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl font-black tracking-tight">پنل مشاوره و تخمین آماری قبولی</h1>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="p-5 bg-white/[0.03] backdrop-blur-xl rounded-[32px] border border-white/10 text-center min-w-[140px] shadow-2xl">
                <span className="block text-[10px] text-slate-500 font-black mb-1 uppercase tracking-tighter">{stat.label}</span>
                <span className={`text-3xl font-black ${stat.color}`}>{toPersianNum(stat.val)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Control Bar */}
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-xl flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-2">
          <select 
            ...
            className="px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-[10px] font-black outline-none cursor-pointer hover:bg-slate-100 transition-colors"
          >
              ...
              {filteredBenchmarks.map((b, i) => {
                ...
                return (
                  <motion.div 
                    ...
                    className="bg-white border border-slate-100 p-6 rounded-[40px] shadow-lg hover:shadow-2xl hover:border-indigo-100 transition-all group overflow-hidden relative"
                  >
